...
function Navbar({ children }) {
  return <nav className="bg-gradient-to-r from-blue-900 to-green-800 text-white py-6 px-8 shadow-lg">{children}</nav>;
}

function NavItem({ href, children }) {
  return (
    <a href={href} className="block p-2 text-gray-300 hover:text-white transition-colors duration-200">
      {children}
    </a>
  );
}
          ...
          <div className="flex items-center justify-between max-w-7xl mx-auto">
            ...
            <div>
              <MotionButton
                ...
                className="bg-gray-200 text-gray-800 hover:bg-gray-300 transition-all duration-200 rounded-xl p-2 px-4 py-2"
                aria-label="Logout"
              >
                Logout
              </MotionButton>
